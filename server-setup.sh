...
add_docker_tools() {
    ...
    case $install_choice in
        y|Y)
            ...
            # 下载dlogs.sh脚本
            wget -qO "$tools_folder/dlogs.sh" "https://raw.githubusercontent.com/lldiot/linux-setup/main/dlogs.sh"
            if [ $? -eq 0 ]; then
            ...
            # 下载dcip.sh脚本
            wget -qO "$tools_folder/dcip.sh" "https://raw.githubusercontent.com/lldiot/linux-setup/main/dcip.sh"
            if [ $? -eq 0 ]; then
...
install_xanmod_kernel() {
    ...
    # 检查 CPU 支持的指令集级别
    cpu_support_info=$(/usr/bin/awk -f <(wget -qO - https://raw.githubusercontent.com/lldiot/linux-setup/main/check_x86-64_psabi.sh))
    ...
    case $continue_choice in
        y|Y)
            ...
            # 下载内核文件
            wget "https://github.com/lldiot/linux-setup/releases/download/1102/$headers_file"
            wget "https://github.com/lldiot/linux-setup/releases/download/1102/$image_file"
...
display_menu() {
    ...
    echo -e "${BOLD}GitHub：https://github.com/lldiot/linux-setup${RESET}"
